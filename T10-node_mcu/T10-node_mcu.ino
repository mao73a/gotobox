...
void loop() {
    ...
    if (gNavigationState.allowDisplayGyro()){
        if (gNavigationState.getAutoFreezeEnabled()){
            alarmDiodeBlink((vTimeToFreeze<3000 && vTimeToFreeze>50 && (long)(round(millis()/500))%2==0) ||
                      //  (vTimeToFreeze<800 && vTimeToFreeze>100 && (long)(round(millis()/200))%2==0) ||
                        (vTimeToFreeze<=50 && (long)(round(millis()/20))%2==0));
        }
